...
class train_dataset_loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ###########################################################
            # Here is your code
            nedd_aug = random.uniform(0, 1)
            # if contains augmentation
            if nedd_aug >=0.25:
                aug_number = random.uniform(0, 1)
                # if only one augmentation
                if aug_number >= 0.5:
                    augtype_one = random.randint(1,4)
                    if augtype_one == 1:
                ...
                elif 0.25 <= aug_number < 0.5:
                    augtype_two = random.randint(1,6)
                    if augtype_two in [1, 2, 3]:
                        audio = self.augment_wav.additive_noise('noise',audio)
                    if augtype_two in [1, 4, 5]:
                        audio = self.augment_wav.additive_noise('music',audio)
                    if augtype_two in [2, 4, 6]:
                        audio = self.augment_wav.additive_noise('speech',audio)
                    if augtype_two in [3, 5, 6]:
                        audio = self.augment_wav.reverberate(audio)
                elif 0.05 <= aug_number < 0.25:
                    augtype_three = random.randint(1,4)
                    if augtype_three != 4:
                        audio = self.augment_wav.additive_noise('noise',audio)
                    if augtype_three != 3:
                        audio = self.augment_wav.additive_noise('music',audio)
